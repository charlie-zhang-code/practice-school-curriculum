...
WHERE
    t1.status = 0 -- 触发器，如果预约超过10分钟未确认，则自动取消预约
    DELIMITER $ $ CREATE TRIGGER cancel_reservation
AFTER
INSERT
    ON reservations FOR EACH ROW BEGIN IF NEW.start_time < NOW() + INTERVAL 10 MINUTE
    AND NEW.status = 0 THEN
UPDATE
    reservations
SET
    `status` = 3
...
-- 自动触发器，到达预约结束时间时，将座位状态改为空闲，并将预约状态改为已完成
DELIMITER $ $ CREATE TRIGGER update_reservation_status
AFTER
UPDATE
    ON reservations FOR EACH ROW BEGIN IF OLD.end_time < NOW()
    AND OLD.status <> NEW.status THEN -- 将座位状态改为空闲
UPDATE
    seats
SET
    status = 0
